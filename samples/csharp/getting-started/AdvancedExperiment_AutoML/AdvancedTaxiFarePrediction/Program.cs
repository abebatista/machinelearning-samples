...
namespace AdvancedTaxiFarePrediction
{
    internal static class Program
    {
        ...
        private static string BaseDatasetsRelativePath = @"C:\Users\billy\source\repos\machinelearning-samples\datasets";
        
        //private static string TrainDataRelativePath = $"{BaseDatasetsRelativePath}/taxi-fare-train.csv";
        private static string TrainDataRelativePath = $"{BaseDatasetsRelativePath}/SPY-train.csv";
        ...
        private static string TestDataRelativePath = $"{BaseDatasetsRelativePath}/SPY-test.csv";
        ...
        static void Main(string[] args) //If args[0] == "svg" a vector-based chart will be created instead a .png chart
        {
            ...
            //TestSinglePredictionStock(mlContext);
            ...
            PlotRegressionChartStock(mlContext, TestDataPath, 100, args);
        ...
        /// </summary>
        private static RegressionExperimentSettings CreateExperimentSettings(MLContext mlContext,CancellationTokenSource cts)
        {
            var experimentSettings = new RegressionExperimentSettings();
            experimentSettings.MaxExperimentTimeInSeconds = 60; // 3600;
        ...
        private static void TestSinglePredictionStock(MLContext mlContext)
        {
            ConsoleHelper.ConsoleWriteHeader("=============== Testing prediction engine ===============");
        ...
        private static void PlotRegressionChartStock(MLContext mlContext,
                                                string testDataSetPath,
                                                int numberOfRecordsToRead,
                                                string[] args)
        {
            ...
            // Create prediction engine related to the loaded trained model
            var predFunction = mlContext.Model.CreatePredictionEngine<DayQuote, TaxiTripFarePrediction>(trainedModel);
            ...
            using (var pl = new PLStream())
            {
                ...
                // Set axis limits.
                const int xMinLimit = 0;
                const int xMaxLimit = 400; // Rides larger than $35 are not shown in the chart.
                const int yMinLimit = 0;
                const int yMaxLimit = 400;  // Rides larger than $35 are not shown in the chart.
                pl.env(xMinLimit, xMaxLimit, yMinLimit, yMaxLimit, AxesScale.Independent, AxisBox.BoxTicksLabelsAxes);
                ...
                int totalNumber = numberOfRecordsToRead;
                var testData = new TaxiTripCsvReader().GetDataFromCsvDayQuote(testDataSetPath, totalNumber).ToList();
    ...
    public class TaxiTripCsvReader
    {
        ...
        public IEnumerable<DayQuote> GetDataFromCsvDayQuote(string dataLocation, int numMaxRecords)
        {
            IEnumerable<DayQuote> records =
                File.ReadAllLines(dataLocation)
                .Skip(1)
                .Select(x => x.Split(','))
                .Select(x => new DayQuote()
                {
                    Open = float.Parse(x[0]),
                    High = float.Parse(x[1]),
                    Low = float.Parse(x[2]),
                    Close = float.Parse(x[3]),
                    Volume = long.Parse(x[4]),
                    PaymentType = x[5],
                    FareAmount = float.Parse(x[6])
                })
                .Take<DayQuote>(numMaxRecords);
